...
def test_get_list_admin_user(admin_user, saving_study):
    client.set_user(admin_user)
    response = client.get(API_URL)
    assert response.status_code == 200
    json_response = response.json()
    assert json_response["total"] == SavingStudy.objects.count()
...
@pytest.mark.django_db
def test_partial_update_saving_study_admin_user(
    admin_user, saving_study, saving_study_update_request
):
    client.set_user(admin_user)
